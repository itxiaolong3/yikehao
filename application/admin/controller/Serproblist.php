...
class Serproblist extends Base {
    ...
    public function index(){
        if (request()->isPost()){
            $getid=input('id');
            $re=Db::table('problist')->where('id',$getid)->delete();
            if ($re){
                $redata['code']=1;
                $redata['msg']='删除成功';
            ...
            $userlist=Db::table('problist')->select();
    ...
    public function addprob(){
        ...
        if (request()->isPost()){
            ...
            if (empty($getid)){
                //添加
                $re=Db::table('problist')->insert($getinfo);
                if ($re){
                    $redata['code']=1;
                    $redata['msg']='添加成功';
                    echo json_encode($redata);
                ...
                //修改
                $re=Db::table('problist')->where('id',$getid)->update($getinfo);
                if ($re){
                    $redata['code']=1;
                    $redata['msg']='修改成功';
                    echo json_encode($redata);
            ...
            $getid=input('id');
            $getroot=Db::table('problist')->where('id',$getid)->find();
            $this->assign('probinfo',$getroot);
            //权限判断
            $getrootids=session('rootids');
            $idsarr=explode(',',$getrootids);
            if (in_array('6',$idsarr)){
